...
func appendUsageLogIPAddressWhereCondition(conditions []string, args []any, column, ipAddress string) ([]string, []any) {
	ipAddress = strings.TrimSpace(ipAddress)
	if ipAddress == "" {
		return conditions, args
	}
	conditions = append(conditions, fmt.Sprintf("%s ILIKE $%d", column, len(args)+1))
	args = append(args, "%"+escapeLike(ipAddress)+"%")
	return conditions, args
}
...
func appendUsageLogIPAddressQueryFilter(query string, args []any, column, ipAddress string) (string, []any) {
	ipAddress = strings.TrimSpace(ipAddress)
	if ipAddress == "" {
		return query, args
	}
	query += fmt.Sprintf(" AND %s ILIKE $%d", column, len(args)+1)
	args = append(args, "%"+escapeLike(ipAddress)+"%")
	return query, args
}
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendUsageLogIPAddressWhereCondition(conditions, args, "ip_address", filters.IPAddress)
...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	...
	return filters.UserID == 0 &&
		filters.APIKeyID == 0 &&
		filters.AccountID == 0 &&
		strings.TrimSpace(filters.IPAddress) == ""
}
...
func (r *usageLogRepository) GetUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, ipAddress ...string) (results []TrendDataPoint, err error) {
	ipFilter := firstOptionalString(ipAddress)
	if shouldUsePreaggregatedTrend(granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, ipFilter) {
	...
	query, args = appendUsageLogIPAddressQueryFilter(query, args, "ip_address", ipFilter)
...
func shouldUsePreaggregatedTrend(granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, ipAddress string) bool {
...
func (r *usageLogRepository) GetGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, ipAddress ...string) (results []usagestats.GroupStat, err error) {
	...
	query, args = appendUsageLogIPAddressQueryFilter(query, args, "ul.ip_address", firstOptionalString(ipAddress))
	query, args = appendRequestTypeOrStreamQueryFilter(query, args, requestType, stream)
	if billingType != nil {
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, ipAddress string) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = "COALESCE(SUM(COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)), 0) as actual_cost"
	}
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, ipAddress string) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = "COALESCE(SUM(COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)), 0) as actual_cost"
	}
...
func (r *usageLogRepository) GetEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, ipAddress ...string) ([]EndpointStat, error) {
	return r.getEndpointStatsByColumnWithFilters(ctx, "inbound_endpoint", startTime, endTime, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, firstOptionalString(ipAddress))
}

// GetUpstreamEndpointStatsWithFilters returns upstream endpoint statistics with optional filters.
func (r *usageLogRepository) GetUpstreamEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, ipAddress ...string) ([]EndpointStat, error) {
	return r.getEndpointStatsByColumnWithFilters(ctx, "upstream_endpoint", startTime, endTime, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, firstOptionalString(ipAddress))
}
