...
func TestUsageLogRepositoryGetStatsWithFiltersAppliesIPAddressFilter(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}

	filters := usagestats.UsageLogFilters{IPAddress: " 203.0.113.7 "}
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE ip_address ILIKE \\$1.*GROUP BY GROUPING SETS").
		...
		}).AddRow(1, 1, nil, nil, int64(1), int64(2), int64(3), int64(0), int64(0), 0.1, 0.08, 0.07, 10.0))
