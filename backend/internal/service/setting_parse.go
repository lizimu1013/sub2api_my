...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyUserUsageVisibleDays:                      strconv.Itoa(UserUsageVisibleDaysDefault),
		SettingKeyLowBalanceDisplayRateThreshold:            strconv.FormatFloat(LowBalanceDisplayRateThresholdDefault, 'f', 2, 64),
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	if days, err := strconv.Atoi(strings.TrimSpace(settings[SettingKeyUserUsageVisibleDays])); err == nil {
		result.UserUsageVisibleDays = NormalizeUserUsageVisibleDays(days)
	} else {
		result.UserUsageVisibleDays = UserUsageVisibleDaysDefault
	}
