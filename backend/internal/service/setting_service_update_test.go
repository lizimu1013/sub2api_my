...
func TestSettingService_UpdateSettings_RegistrationEmailBlacklist_Normalized(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		RegistrationEmailBlacklist: []string{"Blocked@Example.com", "evil.com", "@EVIL.com"},
	})
	require.NoError(t, err)
	require.Equal(t, `["blocked@example.com","@evil.com"]`, repo.updates[SettingKeyRegistrationEmailBlacklist])
}

func TestSettingService_UpdateSettings_RegistrationEmailBlacklist_Invalid(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		RegistrationEmailBlacklist: []string{"bad@@example.com"},
	})
	require.Error(t, err)
