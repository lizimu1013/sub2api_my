...
func (s *SettingService) GetUserUsageVisibleDays(ctx context.Context) int {
	...
	if err != nil {
		return UserUsageVisibleDaysDefault
	}
...
func (s *SettingService) GetLowBalanceDisplayRateThreshold(ctx context.Context) float64 {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyLowBalanceDisplayRateThreshold)
	if err != nil {
	...
	return NormalizeLowBalanceDisplayRateThreshold(threshold)
}
