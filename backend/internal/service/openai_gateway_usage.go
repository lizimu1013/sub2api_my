...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if !isSubscriptionBilling {
		currentMultiplier := usageLogMultiplier(result, isVideoUsage, cost, multiplier, imageMultiplier, videoMultiplier)
		if adjustedCost, adjustedMultiplier := s.adjustOpenAIBalanceCostForLowBalanceDisplayRate(ctx, user, apiKey, result, billingModels, tokens, serviceTier, cost, currentMultiplier, longContextBillingGate, pricingAt); adjustedCost != nil {
			cost = adjustedCost
	...
	usageLog.RateMultiplier = usageLogMultiplier(result, isVideoUsage, cost, multiplier, imageMultiplier, videoMultiplier)
...
func usageLogMultiplier(result *OpenAIForwardResult, isVideoUsage bool, cost *CostBreakdown, tokenMultiplier, imageMultiplier, videoMultiplier float64) float64 {
...
func (s *OpenAIGatewayService) adjustOpenAIBalanceCostForLowBalanceDisplayRate(
	ctx context.Context,
	user *User,
	apiKey *APIKey,
	result *OpenAIForwardResult,
	...
	if s == nil || user == nil || apiKey == nil || apiKey.Group == nil || currentCost == nil {
		return nil, 0
	}
	if currentCost.ActualCost <= 0 || (currentCost.BillingMode != "" && currentCost.BillingMode != string(BillingModeToken)) {
		return nil, 0
	}
	displayMultiplier := apiKey.Group.DisplayRateMultiplier
	if displayMultiplier <= 0 || displayMultiplier == currentMultiplier {
		return nil, 0
	}
	...
	if !s.userBalanceAtOrBelowThreshold(ctx, user, threshold) {
		return nil, 0
	}
	adjustedCost, err := s.calculateOpenAIRecordUsageCost(ctx, result, apiKey, billingModels, displayMultiplier, displayMultiplier, displayMultiplier, displayMultiplier, tokens, serviceTier, longContextBillingGate, pricingAt)
	if err != nil {
		logger.LegacyPrintf("service.openai_gateway", "low-balance display rate recalculation failed: user=%d group=%d err=%v", user.ID, apiKey.Group.ID, err)
		return nil, 0
	}
	return adjustedCost, displayMultiplier
}

func (s *OpenAIGatewayService) userBalanceAtOrBelowThreshold(ctx context.Context, user *User, threshold float64) bool {
	if user == nil {
		return false
	}
	balance := user.Balance
	if s != nil && s.billingCacheService != nil && (s.billingCacheService.cache != nil || s.billingCacheService.userRepo != nil) {
