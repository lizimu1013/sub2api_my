...
func (h *UsageHandler) parseUserUsageFilters(c *gin.Context, requireRange bool) (*userUsageFilters, bool) {
	...
	startPtr, endPtr = applyUsageVisibilityFilterRange(startPtr, endPtr, visibilityPolicy)
...
func (h *UsageHandler) DashboardStats(c *gin.Context) {
	...
	response.Success(c, userDashboardStatsFromUsageStats(stats))
}
...
func (h *UsageHandler) DashboardTrend(c *gin.Context) {
	...
	response.Success(c, gin.H{
		"trend":       userTrendDataFromUsageStats(trend),
...
func (h *UsageHandler) DashboardSnapshotV2(c *gin.Context) {
	...
	if includeTrend {
		...
		resp["trend"] = userTrendDataFromUsageStats(trend)
	}
	if includeModels {
...
func userEndpointStatsFromUsageStats(stats []usagestats.EndpointStat) []userEndpointStat {
	out := make([]userEndpointStat, 0, len(stats))
	for _, stat := range stats {
		out = append(out, userEndpointStat{
...
func userUsageStatsFromUsageStats(stats *usagestats.UsageStats) *userUsageStats {
	...
	return &userUsageStats{
		...
		Endpoints:                userEndpointStatsFromUsageStats(stats.Endpoints),
	}
}

func userTrendDataFromUsageStats(stats []usagestats.TrendDataPoint) []userTrendDataPoint {
...
func userAPIKeyDailyUsageFromUsageStats(stats []usagestats.APIKeyDailyUsagePoint) []userAPIKeyDailyUsagePoint {
	out := make([]userAPIKeyDailyUsagePoint, 0, len(stats))
	for _, stat := range stats {
...
func (h *UsageHandler) GetMyAPIKeyDailyUsage(c *gin.Context) {
	...
	response.Success(c, gin.H{
		"items":      userAPIKeyDailyUsageFromUsageStats(items),
		"days":       days,
