...
func eventColumns(alias string) string {
	return fmt.Sprintf(`%[1]s.id,%[1]s.job_id,%[1]s.request_id,%[1]s.user_id,%[1]s.username_snapshot,
		...
		%[1]s.scanner_scores,%[1]s.scanner_evidence,%[1]s.scanner_evidence_chunks,%[1]s.scanner_backend,%[1]s.scanner_version,
...
func scanEvent(row rowScanner, withFullPrompt, withExecutionMode bool) (*Event, error) {
	...
	_ = json.Unmarshal(evidenceChunks, &event.ScannerEvidenceChunks)
	result := NormalizedResult{Decision: event.Decision, RiskLevel: event.RiskLevel, Action: event.Action,
		...
		ScannerEvidence: event.ScannerEvidence, ScannerEvidenceChunks: event.ScannerEvidenceChunks}
