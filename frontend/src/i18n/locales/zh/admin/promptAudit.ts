export default {
  promptAudit: {
    ...
    policy: {
      ...
      customPromptTitle: '自定义提示词审计', customPromptEnabled: '启用自定义提示词审计', customPromptHint: '启用后，审计节点会收到系统提示词和 <user_input> 包装内容；可返回 flagged 或 confidence JSON。', customPromptMaxTokens: '自定义审计 max_tokens', customPromptMaxTokensHint: '控制自定义提示词审计的输出预算，默认 512；推理模型如因 reasoning 超限返回空内容，可适当提高。', blockThreshold: '阻断阈值', blockThresholdHint: 'confidence 达到该值时判定为严重并阻断请求。', flagThreshold: '标记阈值', flagThresholdHint: 'confidence 达到该值时标记为需关注事件，但仍放行。', blockHTTPStatus: '拦截响应状态码', blockHTTPStatusHint: '被拦截时返回给客户端的 HTTP 状态码，范围 400-499。', blockMessage: '拦截提示文案', blockMessageHint: '返回给客户端的说明文字，不包含内部风险分类与请求 ID。', systemPrompt: '系统提示词', violationAction: '违规后处理', blockViolation: '直接拒绝请求', fallbackViolation: '降级到指定分组', fallbackGroup: '降级目标分组', chooseFallbackGroup: '请选择目标分组', fallbackHint: '仅当前请求切换分组，不修改 API Key 的原绑定；目标分组需保持启用。', fallbackGroupUnavailable: '已配置的目标分组不存在、停用或不可路由，保存前请重新选择。',
    },
    ...
    events: {
      title: '审计事件', description: '按身份、入口、风险、Hash 和时间复核事件，详情中可查看完整提示词。', decision: '判定', risk: '风险等级', executionMode: '审核方式', executionModes: { blocking: '同步审核', async_audit: '异步审核', unknown: '未知' }, endpoint: '入口', groupId: '分组 ID', userId: '用户 ID', apiKeyId: 'API Key ID', keyword: '关键词',
