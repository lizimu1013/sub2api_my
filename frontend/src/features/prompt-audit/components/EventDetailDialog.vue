...
function formatDecisionAction(decision: string, action: string): string {
  ...
  if (decision === 'flag' && action === 'Warn' && props.event?.categories.includes('request_canceled')) {
    return `${t('admin.promptAudit.events.requestCanceled')} · ${t('admin.promptAudit.events.notDispatched')}`
  }
